...
export default class ItemSheet5e extends ItemSheet {
  ...
  _getItemProperties(item) {
    ...
    if ( item.type === "weapon" ) {
      props.push(...Object.entries(item.data.properties)
        .filter(e => e[1] === true)
        .map(e => CONFIG.FANHUNTER.weaponProperties[e[0]]));
    }
      ...
      props.push(CONFIG.FANHUNTER.equipmentTypes[item.data.armor.type]);
    ...
    if ( item.data.actionType ) {
      props.push(CONFIG.FANHUNTER.itemActionTypes[item.data.actionType]);
    }
  ...
  _onConfigureClassSkills(event) {
    ...
    const choices = skills.choices && skills.choices.length ? skills.choices : Object.keys(CONFIG.FANHUNTER.skills);
    const a = event.currentTarget;
    ...
    new TraitSelector(this.item, {
      ...
      choices: Object.entries(CONFIG.FANHUNTER.skills).reduce((obj, e) => {
