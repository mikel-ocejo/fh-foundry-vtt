...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.FANHUNTER.areaTargetTypes);
  }
  ...
  async _handleResourceConsumption({isCard=false, isAttack=false}={}) {
    ...
    const typeLabel = CONFIG.FANHUNTER.abilityConsumptionTypes[consume.type];
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.FANHUNTER.weaponTypes[data.weaponType],
    );
  }
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.FANHUNTER.abilities[data.ability] || null,
      CONFIG.FANHUNTER.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = CONFIG.FANHUNTER.spellScrollIds[level];
