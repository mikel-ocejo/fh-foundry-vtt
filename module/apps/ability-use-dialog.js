...
export default class AbilityUseDialog extends Dialog {
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const canUpcast = (lvl > 0) && CONFIG.FANHUNTER.spellUpcastModes.includes(itemData.preparation.mode);

    // Determine the levels which are feasible
    let lmax = 0;
    const spellLevels = Array.fromRange(10).reduce((arr, i) => {
      if ( i < lvl ) return arr;
      const label = CONFIG.FANHUNTER.spellLevels[i];
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    // Other Items
      ...
      return game.i18n.format("FANHUNTER3E.AbilityUseNormalHint", {
        ...
        value: uses.value,
        max: uses.max,
        per: CONFIG.FANHUNTER.limitedUsePeriods[uses.per]
      });
    }
  }
