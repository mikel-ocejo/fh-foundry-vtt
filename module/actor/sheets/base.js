...
export default class ActorSheet5e extends ActorSheet {
  ...
  getData() {
    ...
    for ( let [a, abl] of Object.entries(data.actor.data.abilities)) {
      ...
      abl.hover = CONFIG.FANHUNTER.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.FANHUNTER.abilities[a];
    }

    // Skills
    if (data.actor.data.skills) {
      for ( let [s, skl] of Object.entries(data.actor.data.skills)) {
        skl.ability = CONFIG.FANHUNTER.abilityAbbreviations[skl.ability];
        skl.icon = this._getProficiencyIcon(skl.value);
        skl.hover = CONFIG.FANHUNTER.proficiencyLevels[skl.value];
        skl.label = CONFIG.FANHUNTER.skills[s];
      }
    }
  ...
  _prepareTraits(traits) {
    const map = {
      "dr": CONFIG.FANHUNTER.damageResistanceTypes,
      "di": CONFIG.FANHUNTER.damageResistanceTypes,
      "dv": CONFIG.FANHUNTER.damageResistanceTypes,
      "ci": CONFIG.FANHUNTER.conditionTypes,
      "languages": CONFIG.FANHUNTER.languages,
      "armorProf": CONFIG.FANHUNTER.armorProficiencies,
      "weaponProf": CONFIG.FANHUNTER.weaponProficiencies,
      "toolProf": CONFIG.FANHUNTER.toolProficiencies
    };
  ...
  _prepareSpellbook(data, spells) {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.FANHUNTER.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.FANHUNTER.spellLevels[lvl], levels[sl]);
      }
    }
    if ( levels.pact && levels.pact.max ) {
      registerSection("spell0", 0, CONFIG.FANHUNTER.spellLevels[0]);
      registerSection("pact", sections.pact, CONFIG.FANHUNTER.spellPreparationModes.pact, levels.pact);
    }
    ...
    spells.forEach(spell => {
      ...
      if ( mode in sections ) {
        s = sections[mode];
        if ( !spellbook[s] ){
          registerSection(mode, s, CONFIG.FANHUNTER.spellPreparationModes[mode], levels[mode]);
        }
      }

      // Higher-level spell headings
      else if ( !spellbook[s] ) {
        registerSection(sl, s, CONFIG.FANHUNTER.spellLevels[s], levels[sl]);
      }
  ...
  _onTraitSelector(event) {
    ...
    const choices = CONFIG.FANHUNTER[a.dataset.options];
